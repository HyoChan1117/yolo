...
_log_path = os.path.join("logs", time.strftime("line_counter_dual_%Y%m%d.log"))
logging.basicConfig(
    level=logging.INFO,
...
MODEL_PATH = "human/models/yolo11m.engine"
...
class CamCounter:
    ...
    def _display_loop(self):
        while self.running:
            ...
            for x1, y1, x2, y2, color, _ in active.values():
...
def main():
    ...
    while True:
        ...
        if cached[0] is not None and cached[1] is not None:
            ...
            cv2.imshow("Line Counter Dual (q: 종료)", combined)
